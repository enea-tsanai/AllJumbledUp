...
import AllJumbledUp.Session;
import AllJumbledUp.DbManager;
...
public class MenuController {
    ...
    @FXML
    private ComboBox<GameManager.DifficultyLevel> difficultyLevel;
...
//            settings.getChildren().add(HighScoresTable);
        ...
        ObservableList<GameManager.DifficultyLevel> difficultyOpts =
                FXCollections.observableArrayList(GameManager.DifficultyLevel.values());
        difficultyLevel.setItems(difficultyOpts);
        difficultyLevel.getSelectionModel().selectFirst();

        backgroundMusic.setSelected(gameManager.getBackgroundMusic());
        fxSounds.setSelected(gameManager.getFxSounds());
    }

    @FXML
    private void handleButtonAction(ActionEvent event) {
        gameManager.setDifficultyLevelLevel(difficultyLevel.getSelectionModel().getSelectedItem());
        gameManager.startGame();
        gameManager.showMainGameScene();
    }
    ...
    public void handleBackgroundMusic(ActionEvent event) {
        gameManager.setBackgroundMusic(backgroundMusic.isSelected());
    }

    public void handleFxSounds(ActionEvent event) {
        gameManager.setFxSounds(fxSounds.isSelected());
    }
}
